...
public class ItemPickaxePhantium extends ItemPickaxeGeneric {

	public ItemPickaxePhantium(ToolMaterial material) {
		super(material);
	}
	...
	{
		if (Config.phantiumPickaxeDrop == 0) return super.onBlockDestroyed(stack, worldIn, state, pos, entityLiving);
		if (worldIn.isRemote) return super.onBlockDestroyed(stack, worldIn, state, pos, entityLiving);
		
		Block block = state.getBlock();
		
		if (block.getHarvestLevel(state) <= toolMaterial.getHarvestLevel() && block.getHarvestTool(state).equals("pickaxe"))
		{
			Random random = worldIn instanceof World ? ((World)worldIn).rand : new Random();
			if (random.nextInt(Config.phantiumPickaxeDrop) == 0)
			{
