...
public class ItemShovelPhantium extends ItemShovelGeneric {

	public ItemShovelPhantium(ToolMaterial material) {
		super(material);
	}
	...
	// Dropping Spectral Residue
		...
		{
			if (Config.phantiumShovelDrop == 0) return super.onBlockDestroyed(stack, worldIn, state, pos, entityLiving);
			if (worldIn.isRemote) return super.onBlockDestroyed(stack, worldIn, state, pos, entityLiving);
			
			Block block = state.getBlock();
			
			if (block.getHarvestLevel(state) <= toolMaterial.getHarvestLevel() && block.getHarvestTool(state).equals("shovel"))
			{
				Random random = worldIn instanceof World ? ((World)worldIn).rand : new Random();
				if (random.nextInt(Config.phantiumShovelDrop) == 0)
				{
